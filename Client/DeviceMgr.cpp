...
HRESULT CDeviceMgr::InitDevice(DISPLAY_MODE eMode)
{
	...
	/*LPCWSTR name = L"C://Users\jiwoo\Downloads\EFIGS.fnt";
	if (AddFontResourceEx(name, FR_PRIVATE, NULL) == 0)
	{
		cout << "실패" << endl;
	}*/
	...
	lstrcpy(tFontInfo.FaceName, L"Terminal");
...
void CDeviceMgr::Render_Line_Begin()
{
	m_pLine->Begin();
}

void CDeviceMgr::Render_Line_End()
{
	m_pLine->End();
	// 후면버퍼에 그리기를 마침
	m_pGraphicDev->EndScene();
