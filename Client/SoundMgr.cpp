...
void CSoundMgr::PlaySound(const wstring& wstrSoundKey, SOUNDMGR::CHANNEL_ID eID)
{
...
void CSoundMgr::PlayBGM(const wstring& wstrSoundKey)
{
	...
	FMOD_System_PlaySound(m_pSystem, FMOD_CHANNEL_FREE, iter_find->second,
		FALSE, &m_pChannelArr[SOUNDMGR::BGM]);

	FMOD_Channel_SetMode(m_pChannelArr[SOUNDMGR::BGM], FMOD_LOOP_NORMAL);

	UpdateSound();
}

void CSoundMgr::StopSound(SOUNDMGR::CHANNEL_ID eID)
{
...
{
	for(int i = 0; i < SOUNDMGR::MAX_CHANNEL; ++i)
		FMOD_Channel_Stop(m_pChannelArr[i]);

	UpdateSound();
}

void CSoundMgr::SetVolume(SOUNDMGR::CHANNEL_ID eID, float fVol)
{
...
void CSoundMgr::LoadSoundFile()
{
	...
	int handle = _findfirst("../SMBSound/*.*", &fd);
	...
	char szFullPath[128] = "";
	char szRelativePath[128] = "../SMBSound/";
